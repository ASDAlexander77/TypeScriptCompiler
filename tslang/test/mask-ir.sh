...
#   - the numeric ID after td_, tsrel_, tsret_, tsrelcb_ and tsretcb_, whatever its digit count
#     (a 5-digit td_72136 slips under the 6-digit rule)
...
sed -E 's/([A-Za-z_$]\.?)[0-9]{6,}/\1N/g; s/(td|tsrelcb|tsretcb|tsrel|tsret)_[0-9]+/\1_N/g; s/FH[0-9]+/FHN/g; s/\.[0-9]{6,}\.\.vtbl/.N..vtbl/g; s/(^|[^A-Za-z0-9_.$])-?[0-9]{12,}/\1N/g' "$1" \
  | sed -E '/c"[^"]*(FHN|(td|tsrelcb|tsretcb|tsrel|tsret)_N)/ s/\[[0-9]+ x i8\]/[K x i8]/g'
