...
namespace mlirgen
{
                                             ...
                                             const GenContext &genContext)
    {
        ...
        if (asMethodCall)
        {
            ...
            // `C..new` is generated by this compiler, so it goes through the return path that
            // retains its result (§9.24) and the instance arrives already owned. Say so here,
            // where the callee is known, rather than letting anything downstream guess from the
            // shape of the call - a runtime helper or an imported function returns a heap value
            // with no retain at all, and mistaking one for this would free live memory.
            if (auto *definingOp = newOp.getDefiningOp())
            {
