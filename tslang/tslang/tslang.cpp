...
cl::opt<std::string> llvmlibpath("llvm-lib-path", cl::desc("LLVM library path (or LLVM_LIB_PATH environmental variable). Not needed any more: programs no longer link an LLVM library. Accepted so older scripts keep working"), cl::value_desc("llvmlibpath"), cl::cat(TypeScriptCompilerBuildCategory));
