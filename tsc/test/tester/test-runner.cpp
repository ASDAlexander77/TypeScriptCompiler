...
void createCompileBatchFileGC()
{
    ...
    batFile << "$TSCEXEPATH/tsc --emit=jit --shared-libs=../../lib/libTypeScriptGCWrapper.so $2 2>$FILENAME.li" << std::endl;
    batFile << "/usr/bin/llc-12 --filetype=obj -o=$FILENAME.o $FILENAME.il" << std::endl;
    batFile << "gcc -fPIC -o $FILENAME -L$GCLIBPATH -lgc-lib $FILENAME.o" << std::endl;
    batFile << "del $FILENAME.o" << std::endl;
...
void createCompileBatchFileGCWithRT()
{
    ...
    batFile << "$TSCEXEPATH/tsc --emit=llvm --shared-libs=../../lib/libTypeScriptGCWrapper.so $2 2>$FILENAME.li" << std::endl;
    batFile << "/usr/bin/llc-12 --filetype=obj -o=$FILENAME.o $FILENAME.il" << std::endl;
    batFile << "gcc -fPIC -o $FILENAME -L$GCLIBPATH -L$CLANGLIBPATH -lgc-lib -lclang_rt.builtins-x86_64 $FILENAME.o" << std::endl;
    batFile << "del $FILENAME.o" << std::endl;
    batFile << "./$FILENAME 1> $FILENAME.txt 2> $FILENAME.err" << std::endl;
    batFile.close();
}
