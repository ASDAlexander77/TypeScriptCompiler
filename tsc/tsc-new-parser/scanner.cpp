...
namespace ts
{
    std::map<string, SyntaxKind> Scanner::textToKeyword = {
 ...
 */
    ...
    /* @internal */
    auto Scanner::computePositionOfLineAndCharacter(std::vector<number> lineStarts, number line, number character, string debugText, bool allowEdits) -> number
    {
 ...
 */
    ...
    auto Scanner::scanConflictMarkerTrivia(safe_string &text, number pos, std::function<void(DiagnosticMessage, number, number)> error) -> number
    {
        if (error)
        {
    ...
    auto Scanner::appendCommentRange(number pos, number end, SyntaxKind kind, boolean hasTrailingNewLine, number state, std::vector<CommentRange> comments) -> std::vector<CommentRange>
    {
 ...
 */
    ...
    auto Scanner::appendIfCommentDirective(
