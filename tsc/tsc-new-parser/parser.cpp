...
struct Parser
{
                       ...
                       IncrementalParser::SyntaxCursor syntaxCursor = undefined, boolean setParentNodes = false) -> JsonSourceFile
    {
        ...
        else
        {
            ...
            auto expression = expressions.size() > 1 ? finishNode(factory.createArrayLiteralExpression(expressions), pos)
    ...
    auto parseLiteralLikeNode(SyntaxKind kind) -> LiteralLikeNode
    {
        ...
        LiteralLikeNode node =
            isTemplateLiteralKind(kind)
                ? factory.createTemplateLiteralLikeNode(kind, scanner.getTokenValue(), getTemplateLiteralRawText(kind),
