...
class MLIRRTTIHelperVCWin32
{
  ...
  public:
    ...
    mlir::LogicalResult setStructValue(mlir::Location loc, mlir::Value &tupleValue, mlir::Value value, int index)
    {
        ...
        tupleValue = rewriter.create<mlir_ts::InsertPropertyOp>(loc, tpl, value, tupleValue, rewriter.getArrayAttr(mth.getStructIndexAttrValue(index)));
        return mlir::success();
    }
    ...
    mlir::Value getTupleFromArrayAttr(mlir::Location loc, mlir_ts::TupleType tupleType, mlir::ArrayAttr arrayAttr)
    {
        ...
        for (auto item : arrayAttr.getValue())
        {
            ...
            structVal =
                rewriter.create<mlir_ts::InsertPropertyOp>(loc, tupleType, itemValue, structVal, rewriter.getArrayAttr(mth.getStructIndexAttrValue(position++)));
        }
