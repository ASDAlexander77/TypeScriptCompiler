...
class TypeScriptGCPrinter : public GCMetadataPrinter
{
  public:
...
static GCMetadataPrinterRegistry::Add<TypeScriptGCPrinter> TSGCPRN(TYPESCRIPT_GC_NAME, "typescript garbage collector");
...
void TypeScriptGCPrinter::finishAssembly(Module &M, GCModuleInfo &Info, AsmPrinter &AP)
{
    ...
    OS.SwitchSection(AP.getObjFileLowering().getContext().getELFSection(".note.gc", ELF::SHT_PROGBITS, 0));

    // For each function...
    for (GCModuleInfo::FuncInfoVec::iterator FI = Info.funcinfo_begin(), IE = Info.funcinfo_end(); FI != IE; ++FI)
    {
        ...
        for (GCFunctionInfo::live_iterator LI = MD.live_begin(PI), LE = MD.live_end(PI); LI != LE; ++LI)
        {
