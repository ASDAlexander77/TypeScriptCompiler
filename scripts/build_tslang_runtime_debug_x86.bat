...
@rem __build\tslang-runtime\debug\x86; pass --tslang-lib-path=__build\tslang-runtime\debug
@rem (the parent: for an x86 target the compiler looks in its x86 subdirectory).
pushd %~dp0..
cmake -S tslang\runtime -B __build\tslang-runtime\msbuild\x86\debug -G "Visual Studio 18 2026" -A Win32 -Wno-dev -DTSLANG_GC_INCLUDE=%cd%\3rdParty\gc\x86\debug\include -DTSLANG_MLIR_INCLUDE=%cd%\3rdParty\llvm\x64\debug\include -DCMAKE_INSTALL_PREFIX=%cd%\__build\tslang-runtime\debug\x86
